-- ============================================================
-- 32_atomic_notification_purge.sql
-- CineCraft Connect — Atomic Notification Engine Cleanup
-- ============================================================

-- Atomic Dynamic Purge of all functions named 'send_governance_notification'
DO $$ 
DECLARE 
    r RECORD;
BEGIN
    FOR r IN (
        ...
        WHERE p.proname = 'send_governance_notification'
          AND n.nspname = 'public'
    ) LOOP
        EXECUTE 'DROP FUNCTION ' || r.schema_name || '.' || r.function_name || '(' || r.args || ') CASCADE';
    END LOOP;
END $$;

-- Now create the ONE authoritative version with a completely clean slate
