-- ============================================================================
-- FIX: get_unread_message_previews (Table Name & Column Fix)
-- ============================================================================
-- 1. Table `discussion_room_members` does not exist. Correct table is `room_members`.
-- 2. Column `title` on `project_spaces` does not exist. Using `name` (assumed based on standard patterns).
...
BEGIN
    ...
    WITH all_messages AS (
        ...
        WHERE 
            ...
            AND EXISTS (
                 SELECT 1 FROM public.room_members drm -- CORRECTED FROM discussion_room_members
