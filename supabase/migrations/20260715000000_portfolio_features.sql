-- Create migration for pinned posts and feed algorithm features

-- 1. Add is_pinned to posts table
ALTER TABLE posts ADD COLUMN IF NOT EXISTS is_pinned BOOLEAN DEFAULT false;

-- 2. Add pinned_posts_order to profiles table
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS pinned_posts_order UUID[] DEFAULT '{}';

-- 3. Create indexes for performance
CREATE INDEX IF NOT EXISTS idx_posts_author_pinned ON posts (author_id, is_pinned);
CREATE INDEX IF NOT EXISTS idx_posts_tags_gin ON posts USING gin (tags);

-- 4. Create the algorithmic feed function
...
BEGIN
  ...
  FROM posts p
  WHERE 
    p.page_id IS NULL AND -- only show profile posts in home feed (exclude company posts if they are separate, or keep this matching useHomeFeed.ts filter)
