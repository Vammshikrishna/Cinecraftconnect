-- ============================================================================
-- FIX: get_unread_message_previews (Column Name Fix)
-- ============================================================================
-- The error "column ps.title does not exist" indicates that the `project_spaces` table
-- uses `name` instead of `title`. We need to verify and fix this.
-- Based on standard schemas, project spaces usually have `name`.
...
BEGIN
    ...
    WITH all_messages AS (
        ...
        SELECT 
            ...
            ps.name AS name, -- Changed from ps.title to ps.name (assuming name exists if title does not)
            NULL::text AS avatar,
