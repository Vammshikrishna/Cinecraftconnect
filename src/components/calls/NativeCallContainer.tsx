...
export const NativeCallContainer = ({ roomId, onLeave, roomName = 'Team Discussion', projectId }: NativeCallContainerProps) => {
    ...
    // Portal logic for full-screen overlay
    ...
    return createPortal(
        <div ref={containerRef} className="fixed inset-0 z-[99999] bg-[#202124] text-white flex flex-col font-sans h-[100dvh]">

            {/* --- TOP BAR --- */}
            <div className="h-14 bg-[#1f1f1f] border-b border-[#333] flex items-center justify-between px-4 z-10 shrink-0">
                <div className="flex items-center gap-4">
                    <span className="font-semibold text-lg hidden sm:inline">{roomName}</span>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button
                        variant="ghost" size="sm"
                        className={`text-gray-300 hover:text-white hover:bg-[#333] ${activeTab === 'chat' ? 'bg-[#333] text-white' : ''}`}
                        onClick={() => setActiveTab(activeTab === 'chat' ? null : 'chat')}
                    >
                        <MessageSquare className="w-5 h-5 mr-2" />
                        <span className="hidden sm:inline">Chat</span>
                        {messages.length > 0 && (
                            <span className="ml-1 bg-[#333] flex h-2 w-2 rounded-full relative">
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                {/* --- VIDEO STAGE --- */}
                <div className="flex-1 bg-[#202124] relative overflow-hidden flex flex-col">

                    {/* Screen Share Banner */}
                    {isScreenSharing && screenStream && (
                        <div className="absolute top-4 left-0 right-0 z-20 flex justify-center pointer-events-none">
                            <div className="bg-blue-600 px-4 py-2 rounded-full shadow-lg flex items-center gap-3 pointer-events-auto">
                                <MonitorUp className="w-4 h-4 text-white" />
                                <span className="text-white text-sm font-medium">You are presenting</span>
                                <Button size="sm" variant="secondary" className="h-6 text-xs bg-white text-blue-600 hover:bg-gray-100 border-none" onClick={toggleScreenShare}>Stop</Button>
                            </div>
                        </div>
                    )}

                    {/* Grid Container */}
                    <div className="flex-1 flex items-center justify-center p-2 sm:p-4 overflow-y-auto w-full">
                        <div className="flex flex-wrap justify-center items-center gap-2 sm:gap-4 w-full h-full content-center">

                            {/* Screen Share View */}
                            {isScreenSharing && screenStream ? (
                                <div className="w-full h-full flex items-center justify-center relative">
                                    <video ref={(r) => { if (r) r.srcObject = screenStream }} autoPlay playsInline muted className="max-w-full max-h-full object-contain rounded-lg shadow-2xl" />
                                    <div className="absolute bottom-4 right-4 w-32 h-24 sm:w-48 sm:h-32 shadow-2xl hover:scale-105 transition-transform z-30">
                                        <LocalVideoFrame
                                ...
                                (() => {
                                    ...
                                    // Mobile: Stack 2 vertically, Grid 4.
                                    // Desktop: Grid.
                                    let cellClass = "w-full max-w-sm sm:max-w-3xl aspect-video h-auto"; // 1 person

                                    if (total <= 2) cellClass = "w-full sm:w-[48%] max-h-[45vh] aspect-video object-cover"; // 2 people
                                    else if (total > 2) cellClass = "w-[48%] sm:w-[32%] max-h-[30vh] aspect-video object-cover"; // 3+ people
                ...
                {activeTab && (
                    <div className="w-full sm:w-80 bg-[#1f1f1f] border-l border-[#333] flex flex-col absolute top-0 bottom-0 right-0 z-30 sm:static shadow-2xl animate-in slide-in-from-right">
                        <div className="p-4 border-b border-[#333] flex justify-between items-center bg-[#2b2b2b]">
                            <h3 className="font-semibold text-white">{activeTab === 'chat' ? 'Chat' : 'People'}</h3>
                            <Button variant="ghost" size="icon" onClick={() => setActiveTab(null)} className="h-8 w-8 text-gray-400 hover:text-white">
                                <X className="h-5 w-5" />
                            </Button>
                        </div>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
                                    {messages.map((msg) => (
                                        <div key={msg.id} className="flex flex-col gap-1">
                                            ...
                                            <p className="text-sm text-gray-200 bg-[#333] p-2 rounded-lg border border-[#444] break-words">{msg.content}</p>
                                        </div>
                                    ))}
                                </div>
                                <div className="p-3 border-t border-[#333] flex gap-2">
                                    <Input
                                        className="bg-[#3c4043] border-none text-white rounded-full"
                                        placeholder="Send a message..."
                                        value={chatInput}
                                        onChange={(e) => setChatInput(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && sendMessage()}
                                    />
                                    <Button size="icon" onClick={sendMessage} className="rounded-full bg-blue-600 hover:bg-blue-700">
            ...
            {/* --- BOTTOM CONTROL BAR --- */}
            <div className="h-20 bg-[#1f1f1f] border-t border-[#333] flex items-center justify-center shrink-0 gap-2 sm:gap-4 px-4 relative z-20">
                {/* --- MOBILE LAYOUT (Visible < md) --- */}
                <div className="flex md:hidden items-center justify-between w-full">
                    <ControlButton onClick={onLeave} active={false} inactiveIcon={<PhoneOff className="w-5 h-5" />} danger={true} />
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button className="h-12 w-12 rounded-full bg-[#3c4043] border-none hover:bg-[#4d5155]">
                                    <Smile className="w-5 h-5" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent side="top" className="bg-[#333] border-[#444] text-white p-2 w-64 grid grid-cols-4 gap-2 z-[100000]">
                                {EMOJIS.map(e => (
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" size="icon" className="text-gray-300 h-10 w-10"><MoreHorizontal className="w-6 h-6" /></Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="bg-[#333] border-[#444] text-white z-[100000]">
                                <DropdownMenuItem onClick={toggleHandRaise} className="min-h-[48px] text-base">
                                    <Hand className={`w-5 h-5 mr-3 ${isHandRaised ? 'text-yellow-400' : ''}`} /> {isHandRaised ? 'Lower Hand' : 'Raise Hand'}
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={toggleScreenShare} className="min-h-[48px] text-base">
                                    <MonitorUp className="w-5 h-5 mr-3" /> {isScreenSharing ? 'Stop Sharing' : 'Share Screen'}
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setShowSettings(true)} className="min-h-[48px] text-base">
                ...
                <div className="hidden md:flex items-center justify-between w-full inset-0 absolute px-6 pointer-events-none">
                    {/* Left: Info */}
                    <div className="flex items-center gap-4 pointer-events-auto">
                        <span className="text-sm text-gray-400 select-none">{format(new Date(), 'h:mm a')}</span>
                        <div className="h-4 w-[1px] bg-gray-600 mx-2"></div>
                        <span className="text-sm font-medium truncate max-w-[200px] select-none">{roomName}</span>
                    </div>

                    {/* Center: Controls */}
                    <div className="flex items-center gap-3 absolute left-1/2 transform -translate-x-1/2 pointer-events-auto">
                        <ControlButton onClick={toggleMute} active={!isMuted} activeIcon={<Mic className="w-5 h-5" />} inactiveIcon={<MicOff className="w-5 h-5" />} danger={isMuted} />
                        <ControlButton onClick={toggleVideo} active={!isVideoOff} activeIcon={<Video className="w-5 h-5" />} inactiveIcon={<VideoOff className="w-5 h-5" />} danger={isVideoOff} />
                        ...
                        <ControlButton onClick={toggleScreenShare} active={isScreenSharing} activeIcon={<MonitorUp className="w-5 h-5 text-black" />} inactiveIcon={<MonitorUp className="w-5 h-5" />} activeClass="bg-blue-300 hover:bg-blue-400" />
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent side="top" className="bg-[#333] border-[#444] text-white p-2 flex gap-2 z-[100000]">
                                {EMOJIS.map(e => (
                                    <button key={e} onClick={() => sendReaction(e)} className="text-2xl hover:scale-125 transition-transform p-1">{e}</button>
                                ))}
                            </DropdownMenuContent>
                        ...
                        <Button variant="destructive" className="h-12 w-16 rounded-full bg-red-600 hover:bg-red-700 ml-2" onClick={onLeave}>
                    ...
                    <div className="flex items-center gap-2 pointer-events-auto">
                        <Button variant="ghost" size="icon" onClick={() => setShowSettings(true)} className="text-gray-300 hover:text-white hover:bg-[#333]"><Settings className="w-5 h-5" /></Button>
                        <Button variant="ghost" size="icon" onClick={toggleFullscreen} className="text-gray-300 hover:text-white hover:bg-[#333]"><Maximize className="w-5 h-5" /></Button>
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" size="icon" className="text-gray-300"><MoreHorizontal className="w-5 h-5" /></Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="bg-[#333] border-[#444] text-white z-[100000]">
                                <DropdownMenuItem onClick={() => setActiveTab('people')} className="cursor-pointer"><Users className="w-4 h-4 mr-2" /> People</DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                    </div>
                </div>
...
const SettingsDialog = ({ open, onOpenChange, currentStream: _currentStream, onDeviceChange }: {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="bg-[#333] border-[#444] text-white z-[100000]">
                <DialogHeader>
