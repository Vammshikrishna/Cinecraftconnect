...
const FeedVendorCard = ({ vendor }: FeedVendorCardProps) => {
    return (
        <Link to={`/vendors/${vendor.id}`} className="block h-full">
            <div className="group relative overflow-hidden rounded-[24px] border border-white/10 bg-white/40 dark:bg-black/40 backdrop-blur-xl hover:shadow-[0_8px_30px_-5px_rgba(var(--primary),0.2)] hover:-translate-y-1 hover:border-primary/50 transition-all duration-500 h-full flex flex-col items-center text-center p-0">
                {/* Decorative Header */}
